...
import com.example.demoarchitecture.ui.level1.Level1ViewModel.State.ScreenState

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun Level1Screen(
    onNavigateBack: () -> Unit,
    viewModel: Level1ViewModel = hiltViewModel()
) {
        ...
        when (val screen = state.screenState) {
            is ScreenState.Idle -> {
                Column(
                    ...
                    Button(onClick = { viewModel.sendIntent(Level1ViewModel.Intent.LoadData) }) {
                        Text("Load")
                    }
