...
import com.example.demoarchitecture.ui.level2.Level2ViewModel.State.ScreenState

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun Level2Screen(
    onNavigateBack: () -> Unit,
    viewModel: Level2ViewModel = hiltViewModel()
) {
        ...
        when (val screen = state.screenState) {
            is ScreenState.Idle -> {
                Column(
                    ...
                    Button(onClick = { viewModel.sendIntent(Level2ViewModel.Intent.LoadData) }) {
                        Text("Load")
                    }
